    ...
    )
{
    ...
    if (check_inputs)
    {
         ...
         */
        if (
            tx.are_inputs_standard_2() == false &&
...
std::pair<bool, std::string> transaction_pool::accept_2(
	transaction & tx, bool * missing_inputs
    )
{
    if (missing_inputs)
    {
    ...
    /**
     * Workaround for bug in older Bitcoin(Core) code.
     * @note Can be removed when no longer viable.
     */
    if (
    	static_cast<std::time_t> (tx.time_lock()) >
        std::numeric_limits<std::int32_t>::max()
        )
    {
        return std::make_pair(
        	false, "not accepting time lock beyond 2038 yet"
        );
    }
    ...
    /**
     * Do we already have it in the transaction pool?
     */
    auto hash = tx.get_hash();
     ...
     * We always check the inputs.
     */
    auto check_inputs = true;
    
    if (check_inputs == true)
    {
        if (coins_cache::instance().have_coins(hash) == true)
     	{
			return std::make_pair(false, "already have coins in cache");
      	}
       
		for (auto & i : tx.transactions_in())
		{
			if (
				coins_cache::instance().have_coins(
                i.previous_out().get_hash()) == false
          	  )
			{
   				if (missing_inputs)
       			{
           		...
           		return std::make_pair(
               		false, "already have coins (previous) in cache"
               );
			}
		}
  
  		if (tx.have_inputs() == false)
    	{
			return std::make_pair(false, "inputs already spent");
     	}
        ...
        {
            log_error(
                "Transaction pool accept failed, nonstandard transaction "
                "input."
            );
        ...
        auto fees = tx.get_value_in_2() - tx.get_value_out();
         ...
         * prevent CPU exhaustion denial-of-service attacks.
         */
        if (tx.check_inputs(false, false) == false)
        {
            log_debug(
                "Transaction pool check inputs failed " <<
                hash.to_string().substr(0, 10) << "."
            );
            
            return std::make_pair(false, "check inputs failed");
        }
	}
    ...
    wallet_manager::instance().sync_with_wallets(tx, nullptr, true);
