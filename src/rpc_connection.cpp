    ...
    )
{
    if (globals::instance().state() == globals::state_started)
    {
        ...
        if (request.method == "addendpoint")
        {
            response = json_addendpoint(request);
        }
        else if (request.method == "banip")
        {
            response = json_banip(request);
        }
        ...
        {
            response = json_getblockchaininfo(request);
        }
        ...
        else if (request.method == "gettxoutsetinfo")
        {
            response = json_gettxoutsetinfo(request);
        }
        ...
        else if (request.method == "setnetworktcpbandwidthmaximum")
        {
            response = json_setnetworktcpbandwidthmaximum(request);
        }
    ...
    )
{
    if (auto transport = rpc_transport_.lock())
    {
        ...
        ;
        http_response += "Server: blockchain JSON-RPC 2.0\r\n";
        http_response += "\r\n";
...
rpc_connection::json_rpc_response_t rpc_connection::json_addendpoint(
    const json_rpc_request_t & request
    )
{
    json_rpc_response_t ret;
    ...
    try
    {
        if (request.params.size() == 1)
        {
             ...
             * Get the endpoint.
             */
            auto endpoint = request.params.front().second.get<std::string> ("");
            ...
            /**
             * Attempt to allocate the endpoint, if the address format is wrong
             * this will throw and be caught below.
             */
            boost::asio::ip::tcp::endpoint ep(
                boost::asio::ip::address::from_string(parts[0]),
                std::stoi(parts[1])
            );
            
            /**
             * Convert the endpoint into a protocol::network_address_t
             * structure.
             */
            auto network_addr = protocol::network_address_t::from_endpoint(ep);
            
            /**
             * Create a local source protocol::network_address_t.
             */
            auto network_addr_src = protocol::network_address_t::from_endpoint(
                boost::asio::ip::tcp::endpoint(
                boost::asio::ip::address::from_string("127.0.0.1"), 0)
            );
            
            log_info(
                "RPC connection added endpoint = " << ep << "."
            );
            
            /**
             * Add to the address manager.
             */
            if (stack_impl_.get_address_manager() != nullptr)
            {
                stack_impl_.get_address_manager()->add(
                    network_addr, network_addr_src
                );
            }
             ...
             */
            if (stack_impl_.get_tcp_connection_manager() != nullptr)
            {
                stack_impl_.get_tcp_connection_manager()->connect(ep);
            }
    ...
    {
        log_error(
            "RPC Connection failed to create json_addendpoint, what = " <<
            e.what() << "."
        );
...
rpc_connection::json_rpc_response_t rpc_connection::json_banip(
    const json_rpc_request_t & request
    )
{
    json_rpc_response_t ret;
    ...
    try
    {
        ...
        else if (request.params.size() == 1)
        {
            ...
            /**
             * Ban the IP address for 24 hours.
             */
            network::instance().ban_address(addr.to_string());
    ...
    {
        log_error(
            "RPC Connection failed to create json_banip, what = " <<
            e.what() << "."
        );
...
rpc_connection::json_rpc_response_t rpc_connection::json_getblockchaininfo(
    const json_rpc_request_t & request
    )
{
    json_rpc_response_t ret;
    ...
    try
    {
        ...
        {
            ret.result.put(
                "chain", (constants::test_net ? "testnet" : "main"),
                rpc_json_parser::translator<std::string> ()
            );
            ...
            ret.result.put(
                "hashes",
                globals::instance(
                ).peer_headers_first_heights_and_hashes().size()
            );
        	
            ret.result.put(
                "bestblockhash", stack_impl::get_block_index_best(
                )->get_block_hash().to_string(),
                rpc_json_parser::translator<std::string> ()
            );
            ret.result.put("difficulty", stack_impl_.difficulty());
            ret.result.put(
                "mediantime", stack_impl::get_block_index_best(
                )->get_median_time_past()
            );
            
            /**
             * @note Possibly add verificationprogress.
             */
            
            /**
             * Get the target hash for the best block.
             */
            auto hash_target =
                big_number().set_compact(stack_impl::get_block_index_best(
                )->get_block_header().header().bits).get_sha256()
            ;
 
            /**
             * @note Should be chainwork not the hash target.
             */
            ret.result.put(
                "chainwork", hash_target.to_string(),
                rpc_json_parser::translator<std::string> ()
            );
            
            ret.result.put("pruned", false);
            
            boost::property_tree::ptree pt_bip9_softforks;
            
            version_bits::deployment_t deployment_type =
                version_bits::deployment_type_testdummy
            ;
            
            for (auto & i : version_bits::instance().parameters().deployments)
            {
                boost::property_tree::ptree pt_deployment;
                
                auto state_threshold =
                    version_bits::instance().version_bits_state(
                    stack_impl::get_block_index_best(),
                    version_bits::instance().parameters(),
                    deployment_type,
                    version_bits::instance().threshold_condition_cache()
                );
                
                switch (state_threshold)
                {
                    case version_bits::threshold_defined:
                    {
                        pt_deployment.put(
                            "status", "defined",
                            rpc_json_parser::translator<std::string> ()
                    ...
                    break;
                    case version_bits::threshold_started:
                    {
                        pt_deployment.put(
                            "status", "started",
                            rpc_json_parser::translator<std::string> ()
                    ...
                    break;
                    case version_bits::threshold_locked_in:
                    {
                        pt_deployment.put(
                            "status", "locked_in",
                            rpc_json_parser::translator<std::string> ()
                    ...
                    break;
                    case version_bits::threshold_active:
                    {
                        pt_deployment.put(
                            "status", "active",
                            rpc_json_parser::translator<std::string> ()
                    ...
                    break;
                    case version_bits::threshold_failed:
                    {
                        pt_deployment.put(
                            "status", "failed",
                            rpc_json_parser::translator<std::string> ()
                ...
                if (state_threshold == version_bits::threshold_started)
                {
                    pt_deployment.put("bit", i.bit);
                }
                
                auto state_since =
                    version_bits::instance().version_bits_state_since_height(
                    stack_impl::get_block_index_best(),
                    version_bits::instance().parameters(), deployment_type,
                    version_bits::instance().threshold_condition_cache()
                );
                
                pt_deployment.put("startTime", i.start_time);
                pt_deployment.put("timeout", i.timeout);
                pt_deployment.put("since", state_since);

                /**
                 * In additiojn to version_bits::threshold_started we insert
                 * statistics for version_bits::threshold_locked_in.
                 */
                if (
                    state_threshold == version_bits::threshold_started ||
                    state_threshold == version_bits::threshold_locked_in
                    )
                {
                    auto stats = version_bits::instance().version_bits_statistics(
                        stack_impl::get_block_index_best(),
                        version_bits::instance().parameters(), deployment_type
                    );
                    
                    boost::property_tree::ptree pt_stats;
                    
                    pt_stats.put("period", stats.period);
                    pt_stats.put("threshold", stats.threshold);
                    pt_stats.put("elapsed", stats.elapsed);
                    pt_stats.put("count", stats.count);
                    pt_stats.put(
                        "possible", (stats.possible ? "true" : "false")
                    );
                    
                    pt_deployment.push_back(
                        std::make_pair("statistics", pt_stats)
                    );
                }
                
                pt_bip9_softforks.push_back(std::make_pair(
                    version_bits::instance().deployment_type_to_string(
                    deployment_type), pt_deployment)
                );
                
                deployment_type =
                    static_cast<version_bits::deployment_t> (
                    deployment_type + 1)
                ;
            }

            ret.result.put_child("bip9_softforks", pt_bip9_softforks);
        }
        else
        {
            auto pt_error = create_error_object(
    ...
    )
{
    ...
    try
    {
            ...
            )
        {
            ...
            blk = block::create_new(globals::instance().wallet_main());
         ...
         */
        ret.result.put("sigoplimit", block::get_maximum_size() / 50);
        
        /**
         * Put sizelimit into property tree.
         */
        ret.result.put("sizelimit", block::get_maximum_size());
...
rpc_connection::json_rpc_response_t rpc_connection::json_gettxoutsetinfo(
	const json_rpc_request_t & request
	)
{
    json_rpc_response_t ret;
    ...
    try
    {
    	coins::statistics_t coins_statistics;
     
     	if (coins_database::instance().get_stats(coins_statistics) == true)
      	{
            ret.result.put(
                "bestblock",
                coins_cache::instance().block_index_best()->get_block_hash(
                ).to_string(),
                rpc_json_parser::translator<std::string> ()
            );
            ret.result.put("transactions", coins_statistics.transactions);
            ret.result.put("txouts", coins_statistics.transaction_outputs);
            ret.result.put("bytes_serialized", coins_statistics.encoded_size);
        }
        else
        {
        	ret.result.put("", "null");
    ...
    {
        log_error(
            "RPC Connection failed to create json_gettxoutsetinfo, what = " <<
            e.what() << "."
        );
    }
...
rpc_connection::json_rpc_response_t
    rpc_connection::json_setnetworktcpbandwidthmaximum(
    const json_rpc_request_t & request
    )
{
    json_rpc_response_t ret;
    ...
    try
    {
        if (request.params.size() == 2)
        {
            ...
            for (auto & i : request.params)
            {
                if (index == 0)
                {
                    read = i.second.get<std::size_t> ("");
                }
                else if (index == 1)
                {
                    write = i.second.get<std::size_t> ("");
                }
             ...
             */
            read = std::max(static_cast<std::size_t> (8192), read);
            
            /**
             * Enforce an 8 KB/s write minimum.
             */
            write = std::max(static_cast<std::size_t> (8192), write);
  
            log_info(
                "RPC connection setnetworktcpbandwidthmaximum, read = " <<
             ...
             */
            stack_impl_.get_configuration().set_network_tcp_read_maximum(
                read
            );
             ...
             */
            stack_impl_.get_configuration().set_network_tcp_write_maximum(
                write
            );
             ...
             */
            stack_impl_.get_configuration().save();
    ...
    {
        log_error(
            "RPC Connection failed to create "
            "json_setnetworktcpbandwidthmaximum, what = " << e.what() << "."
        );
    }
        ...
        if (request.params.size() >= 2)
        {
            try
            {
                ...
                for (auto & i : pt_addresses_and_amounts)
                {
                    ...
                    if (addr.is_valid())
                    {
                        ...
                        else
                        {
                             ...
                             */
                            if (amount < globals::instance().transaction_fee())
                            {
                                auto pt_error = create_error_object(
                ...
                auto success =
                    ...
                    to_send, wtx, k, required_fee, filter, 0
                );
    ...
    )
{
    ...
    for (auto & i : transactions)
    {
        ...
        if (
            wtx.is_coin_base() || wtx.is_final() == false
            )
        {
            continue;
        }
