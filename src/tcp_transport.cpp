...
token_bucket tcp_transport::g_token_bucket_read(64000, 128000);
token_bucket tcp_transport::g_token_bucket_write(64000, 128000);
...
void tcp_transport::do_read()
{
    if (m_state == state_connected)
    {
        ...
        else
        {
            ...
            read_retry_timer_.async_wait(strand_.wrap(
                [this, self](boost::system::error_code ec)
            {
                if (ec)
                {
...
void tcp_transport::do_write(const char * buf, const std::size_t & len)
{
    if (m_state == state_connected)
    {
        ...
        else
        {
            ...
            write_retry_timer_.async_wait(strand_.wrap(
                [this, self](boost::system::error_code ec)
            {
                if (ec)
                {
...
void tcp_transport::tick(const boost::system::error_code & ec)
{
    if (ec)
    {
        // ...
