...
void tcp_connection_manager::connect()
{
    /**
     * Start the timer.
     */
...
void tcp_connection_manager::disconnect()
{
    timer_.cancel();
...
std::size_t tcp_connection_manager::minimum_tcp_connections()
{
    ...
    {
        if (
            globals::instance().spv_block_last() == nullptr ||
...
#define BCASH_STRESS_TEST 1
    ...
    /**
     * A single rotating peer is used to synchronize block headers first.
     */
    if (globals::instance().peer_use_headers_first_chain_sync() == true)
    {
        if (
            globals::instance().peer_headers_first_block_last() == nullptr ||
...
#if (defined BCASH_STRESS_TEST && BCASH_STRESS_TEST)
		return bcash_stress_minimum_tcp_connections;
#else
		return utility::is_initial_block_download() ? 8 : 8;
#endif // BCASH_STRESS_TEST
    }
...
void tcp_connection_manager::tick(const boost::system::error_code & ec)
{
    ...
    else
    {
        ...
        std::vector< std::map<std::string, std::string> > vector_pairs;
        
        /**
         * Callback connection (peer) statistics.
         */
        for (auto & i : m_tcp_connections)
        {
            /**
            ...
            {
                /**
                 * Set the protocol.version.version
                 */
                pairs["protocol.version.version"] = std::to_string(
                    connection->protocol_version()
                );
                
                /**
                 * Format the services into a human readable string.
                 */
                std::string protocol_version_services;
                
                if (
                    (connection->protocol_version_services() &
                    protocol::operation_mode_getutxo)
                    )
                {
                    protocol_version_services += "getutxo";
                }
                ...
                {
                    if (protocol_version_services.size() > 0)
                    {
                        protocol_version_services += "|";
                    }
                    
                    protocol_version_services += "bloom";
                }
                
                if (
                    (connection->protocol_version_services() &
                    protocol::operation_mode_witness)
                    )
                {
                    if (protocol_version_services.size() > 0)
                    {
                    ...
                    protocol_version_services += "witness";
                }
                
                if (
                    (connection->protocol_version_services() &
                    protocol::operation_mode_xthin)
                    )
                {
                    if (protocol_version_services.size() > 0)
                    {
                    ...
                    protocol_version_services += "xthin";
                }
                
                if (
                    (connection->protocol_version_services() &
                    protocol::operation_mode_2x)
                    )
                {
                    if (protocol_version_services.size() > 0)
                    {
                    ...
                    protocol_version_services += "2x";
                }
                
                /**
                 * Set the protocol.version.services
                 */
                pairs["protocol.version.services"] =
                    protocol_version_services
                ;
                 ...
                 */
                pairs["protocol.version.timestamp"] = std::to_string(
                    connection->protocol_version_timestamp()
                );
                
                /**
                 * Per IETF IPv6 addresses SHOULD be enclosed in [].
                 * @note Format for Literal IPv6 Addresses in URL's
                 * @link https://www.ietf.org/rfc/rfc2732.txt
                 */
                if (connection->protocol_version_addr_src().is_ipv6() == true)
                {
                    /**
                     * Set the protocol.version.addr_src
                     */
                    pairs["protocol.version.addr_src"] =
                        "[" + connection->protocol_version_addr_src(
                        ).ipv4_mapped_address().to_string() + "]:" +
                        std::to_string(connection->protocol_version_addr_src(
                        ).port)
                    ;
                }
                else
                ...
                /**
                 * Set the protocol.version.addr_dst
                 */

                // ...
                
                /**
                 * Set the protocol.version.nonce
                 */
                
                // ...
                
                /**
                 * Set the protocol.version.user_agent
                 */
                pairs["protocol.version.user_agent"] =
                    connection->protocol_version_user_agent()
                ;
                
                /**
                 * Set the protocol.version.start_height
                 */
                pairs["protocol.version.start_height"] = std::to_string(
                 ...
                 */
                pairs["protocol.version.relay"] = std::to_string(
                    connection->protocol_version_relay()
                );
                
                if (auto transport = connection->get_tcp_transport().lock())
                {
                    /**
                     * Per IETF IPv6 addresses SHOULD be enclosed in [].
                     * @note Format for Literal IPv6 Addresses in URL's
                	...
                	if (
                        ...
                        )
                 	{
                        ...
                        pairs["peer.endpoint"] =
                            ...
                            transport->socket().remote_endpoint().port());
                        ;
                  	}
                   	else
                    {
                ...
                /**
                 * Set the peer.direction
                 */
                pairs["peer.direction"] = std::to_string(
                    connection->direction()
                );
                 ...
                 */
                pairs["peer.identifier"] = std::to_string(
                    connection->identifier()
                );
                
                /**
                 * Set the peer.rtt
                 * @note We skew the RTT slightly.
                 */
                pairs["peer.rtt"] = std::to_string(
                    connection->rtt() + (std::rand() % 6)
                );
                
                if (globals::instance().is_client_spv() == true)
                {
                    /**
                     * Set the peer.ban_score
                     */
                    pairs["peer.ban_score"] = std::to_string(
                        connection->spv_dos_score()
                    );
                }
                else
                {
                ...
                {
                    /**
                     * Set the peer.bytes_total_read
                     */
                    pairs["peer.bytes_total_read"] = std::to_string(
                        t->bytes_total_read()
                    );
                    
                    /**
                     * Set the peer.bytes_total_write
                     */
                    pairs["peer.bytes_total_write"] = std::to_string(
                        t->bytes_total_write()
                    );
                    
                    /**
                     * Set the peer.tcp.total_read
                     */
                    pairs["peer.tcp.total_read"] = network::format_byte_string(
                        t->bytes_total_read()
                    );
                    
                    /**
                     * Set the peer.tcp.total_write
                     */
                    pairs["peer.tcp.total_write"] = network::format_byte_string(
                        t->bytes_total_write()
                    );
                    
                    /**
                     * Set the peer.bytes_per_second_read
                     */
                    pairs["peer.bytes_per_second_read"] = std::to_string(
                        t->bytes_per_second_read()
                    );
                    
                    /**
                     * Set the peer.bytes_per_second_write
                     */
                    pairs["peer.bytes_per_second_write"] = std::to_string(
                        t->bytes_per_second_write()
                    );
                    
                    /**
                     * Set the peer.bandwidth.read
                     */
                    pairs["peer.bandwidth.read"] =
                        network::format_bandwidth_string(
                        t->bytes_per_second_read()
                    );
                    
                    /**
                     * Set the peer.bandwidth.write
                     */
                    pairs["peer.bandwidth.write"] =
                        network::format_bandwidth_string(
                        t->bytes_per_second_write()
                    );
                }
            ...
            vector_pairs.push_back(pairs);
        }
        
        if (vector_pairs.size() > 0)
        {
            /**
             * Callback statistics.
             */
            stack_impl_.get_status_manager()->insert(vector_pairs);
        }
    }
}
    ...
    resolver_.async_resolve(queries.front(),
        strand_.wrap([this, queries](
            ...
            )
            {
                ...
                {
                    boost::asio::ip::tcp::resolver::iterator endpoint_it = it;
                    boost::asio::ip::tcp::resolver::iterator end_it;
                    
                    while (endpoint_it != end_it)
                    {
                        /**
                        ...
                        /**
                         * If the port is 0 set it to the default TCP port.
                         */
                        if (addr.port == 0)
                        {
                            if (constants::test_net == true)
                            {
                                addr.port = protocol::default_tcp_port_testnet;
                            }
                            else
                            {
                                addr.port = protocol::default_tcp_port;
                            }
                        }
                        ...
                        log_debug(
                            "TCP connection manager resolved " <<
                            addr.ipv4_mapped_address() << ":" << addr.port <<
                            "."
                        );
