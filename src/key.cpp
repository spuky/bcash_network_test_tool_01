...
bool key::sign(const sha256 & h, std::vector<std::uint8_t> & signature)
{
    ...
    const EC_GROUP * group = EC_KEY_get0_group(m_EC_KEY);
    ...
    auto size = ECDSA_size(m_EC_KEY);
    
    signature.resize(size);
    
    std::uint8_t * pos = &signature[0];
    
    size = i2d_ECDSA_SIG(sig, &pos);
    ...
    )
{
    ...
    int bits_r = BN_num_bits(sig->r);
    int bits_s = BN_num_bits(sig->s);
    ...
    if (bits_r <= 256 && bits_s <= 256)
    {
        ...
        signature[0] = rec_id + 27 + (m_compressed ? 4 : 0);
...
int key::run_test()
{
    ...
    assert(s1.set_string("5HxWvvfubhXpYYpS3tJkw6fq9jE9j18THftkZjHHfmFiWtmAbrj"));
    assert(s2.set_string("5KC4ejrDjv152FGwP386VD1i2NYc5KkfSMyv1nGy1VGDxGHqVY3"));
    assert(s3.set_string("Kwr371tjA9u2rFSMZjTNun2PXXP3WPZu2afRHTcta6KxEUdm1vEw"));
    assert(s4.set_string("L3Hq7a8FEQwJkW1M2GNKDW28546Vp5miewcCzSqUD9kCAXrJdS3g"));
    ...
    assert("1QFqqMUD55ZV3PJEJZtaKCsQmjLT6JkjvJ" == a1.to_string());
    assert("1F5y5E5FMc5YzdJtB9hLaUe43GDxEKXENJ" == a2.to_string());
    assert("1NoJrossxPBKfCHuJXT4HadJrXRE9Fxiqs" == a1c.to_string());
    assert("1CRj2HyM1CXWzHAXLQtiGLyggNT9WQqsDs" == a2c.to_string());
