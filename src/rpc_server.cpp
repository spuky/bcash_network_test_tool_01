...
bool rpc_server::open(const std::uint16_t & port)
{
    ...
    try
    {
        ...
        auto rpc_local_whitelist =
            stack_impl_.get_configuration().rpc_local_whitelist()
        ;
        
        /**
         * Configuration file overrides command line.
         */
        if (rpc_local_whitelist.size() > 0)
        {
            args["rpc-allow-ips"] = rpc_local_whitelist;
        }
