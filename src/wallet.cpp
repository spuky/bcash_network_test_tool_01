...
key_public wallet::generate_new_key()
{
    ...
    if (m_hd_configuration.id_key_master().is_empty() == false)
    {
        ...
        {
            /**
             * @note Possibly make this an ivar.
             */
...
bool wallet::top_up_key_pool()
{
    ...
    while (m_key_pool.size() < (target_size + 1))
    {
        ...
        m_time_key_pool_did_top_up = std::time(0);
    }
...
void wallet::on_transaction_updated(const sha256 & val)
{
    ...
    if (it != m_transactions.end())
    {
        ...
        status["wallet.transaction.spv_block_height"] =
            std::to_string(wtx.spv_block_height())
        ;
...
void wallet::reaccept_wallet_transactions()
{
    ...
    while (repeat)
    {
        ...
        for (auto & i : m_transactions)
        {
            ...
            if (wtx.is_coin_base() && wtx.is_spent(0))
            {
                continue;
            }
            ...
            else
            {
                 ...
                 */
                if (wtx.is_coin_base() == false)
                {
                    try
                    {
                        wtx.accept_wallet_transaction();
                    }
    ...
    )
{
    ...
    auto existed = m_transactions.count(hash) > 0;
...
void wallet::update_spent(const transaction & tx) const
{
    ...
    for (auto & i : tx.transactions_in())
    {
        ...
        if (it != m_transactions.end())
        {
            ...
            {
                /**
                 * This can happen because of mixed transactions and
                 * is ok.
                 */
                log_debug(
...
const std::time_t & wallet::time_key_pool_did_top_up() const
{
    return m_time_key_pool_did_top_up;
}
...
std::int64_t wallet::get_on_chain_balance() const
{
    ...
    for (auto & i : m_transactions)
    {
        ...
        if (coin.get_depth_in_main_chain() == 0)
        {
            continue;
        }
...
std::map< std::string, std::vector<output> > wallet::get_outputs() const
{
    std::lock_guard<std::recursive_mutex> l1(mutex_);
    
    std::map< std::string, std::vector<output> > ret;
    
    std::vector<output> coins;
    
    std::set<std::int64_t> filter;
    
    available_coins(coins, true, filter, nullptr);
    ...
    for (auto & i : coins)
    {
        ...
        while (
            is_change(out.get_transaction_wallet().transactions_out()[
            out.get_i()]) && out.get_transaction_wallet().transactions_in(
            ).size() > 0 && is_mine(out.get_transaction_wallet(
            ).transactions_in()[0])
            )
        {
            if (
                m_transactions.count(out.get_transaction_wallet(
                ).transactions_in()[0].previous_out().get_hash()) == 0
                )
            {
                break;
            }
            ...
            out = output(
                m_transactions[out.get_transaction_wallet(
                ).transactions_in()[0].previous_out().get_hash()],
                out.get_transaction_wallet().transactions_in(
                )[0].previous_out().n(), 0
            );
        }
        ...
        if (
            script::extract_destination(out.get_transaction_wallet(
            ).transactions_out()[out.get_i()
            ].script_public_key(), address_out) == false
            )
        {
            continue;
        }
        
        ret[address(address_out).to_string()].push_back(out);
    }
    ...
    )
{
    ...
    for (;;)
    {
        ...
        if (
            select_coins(total_value, tx_new.time(), coins, value_in,
            filter, control
            ) == false
            )
        {
         ...
         * least globals::instance().transaction_fee() or until the change
         * becomes zero.
         */
        if (
            fee_out < globals::instance().transaction_fee() && change > 0 &&
            change < constants::cent
            )
        {
            std::int64_t move_to_fee =
                std::min(change,
                globals::instance().transaction_fee() - fee_out)
            ;
         ...
         */
        if (len > transaction::standard_size_maximum)
        {
...
std::pair<bool, std::string> wallet::queue_money(
    const script & script_pub_key, const std::int64_t & value,
    const transaction_wallet & wtx_new,
    std::map<std::string, std::string> & status
    )
{
        ...
        )
    {
        ...
        {
            log_error(
                "Wallet, queue money failed, create transaction failed, "
                "insufficient funds, fee required = " <<
                utility::format_money(fee_required) << "."
            );
    ...
    /**
     * Retain the transaction in memory until it is sent or cancelled.
     */
    transaction_wallet_queued_.first.reset(new transaction_wallet(wtx_new));

    /**
     * Retain the key in memory until it is sent or cancelled.
     */
    transaction_wallet_queued_.second = reserve_key;
    
    log_info(
        "Wallet queued transaction " <<
        wtx_new.get_hash().to_string().substr(0, 8) <<
        ", value = " << utility::format_money(value) << ", fee_required = " <<
        fee_required << "."
    );
     ...
     * Set the wallet.transaction.hash.
     */
    status["wallet.transaction.hash"] = wtx_new.get_hash().to_string();

    /**
     * Set the wallet.transaction.in_main_chain.
     */
    status["wallet.transaction.in_main_chain"] = std::to_string(
        wtx_new.is_in_main_chain()
    );
     ...
     */
    status["wallet.transaction.is_from_me"] =
        std::to_string(wtx_new.is_from_me())
    ;
     ...
     */
    status["wallet.transaction.confirmations"] =
        std::to_string(wtx_new.get_depth_in_main_chain())
    ;
     ...
     */
    status["wallet.transaction.confirmed"] =
        std::to_string(wtx_new.is_confirmed())
    ;
     ...
     */
    status["wallet.transaction.debit"] =
        std::to_string(wtx_new.get_debit())
    ;
     ...
     */
    status["wallet.transaction.net"] =
        std::to_string(wtx_new.get_credit(true) - wtx_new.get_debit())
    ;

    /**
     * Set the wallet.transaction.time.
     */
    status["wallet.transaction.time"] = std::to_string(wtx_new.time());


    /**
     * Set the wallet.transaction.time.
     */
    status["wallet.transaction.fee"] = std::to_string(fee_required);
...
std::pair<bool, std::string> wallet::queue_money_for_destination(
    const destination::tx_t & address, const std::int64_t & value,
    const transaction_wallet & wtx_new,
    std::map<std::string, std::string> & status
    )
{
    ...
    {
        log_error(
            "Wallet, queue money for destination failed, invalid amount."
        );
    ...
    {
        log_error(
            "Wallet, queue money for destination  failed, "
            "insufficient funds = " << utility::format_money(value) <<
            ", fee required = " <<
            utility::format_money(globals::instance().transaction_fee()) <<
            ", balance = " << get_balance() << "."
    ...
    return queue_money(script_pub_key, value, wtx_new, status);
}

bool wallet::send_queued_coins()
{
    if (transaction_wallet_queued_.first)
    {
        /**
         * Commit the transaction.
         */
        auto ret_pair = commit_transaction(
            *transaction_wallet_queued_.first,
            *transaction_wallet_queued_.second
        );
        ...
        {
            log_error(
                "Wallet, send queued coins failed, commit transaction failed, "
                "wallet may need a rescan."
            );
        }
        else
        {
            log_info(
                "Wallet commit queued transaction " <<
                transaction_wallet_queued_.first->get_hash().to_string(
                ).substr(0, 8) << "."
            );
    ...
    {
        log_error(
            "Wallet tried to send queued transaction but none exist."
        );
    }
...
void wallet::cancel_queued_coins()
{
    if (transaction_wallet_queued_.first)
    {
        log_info(
            "Wallet cancelled queued transaction " <<
            transaction_wallet_queued_.first->get_hash(
            ).to_string().substr(0, 8) << "."
        );
        
        transaction_wallet_queued_.first.reset();
        transaction_wallet_queued_.second.reset();
    }
    else
    {
        log_info("Wallet has no queued transactions to cancel.");
    }
}
...
void wallet::tick_flush(const boost::system::error_code & ec)
{
    ...
    else
    {
		...
		/**
		 * @note :SPV: for testing SPV UTXO database.
		 */
#if 0
        std::stringstream ss;
        
        ss << "\nBalance:" << get_balance() << "\n";
        ss << "Pending:" << get_unconfirmed_balance() << "\n";
        ss << "Outputs:\n";
        ...
        for (auto & i : outputs)
        {
            ...
            ss <<
                "\t" << i.first << ":" << utility::format_money(sum) <<
            std::endl;
        }
