...
void script_checker_queue::start()
{
    ...
    if (state_ == state_stopped)
    {
         ...
         */
        cores = (std::min)(
            static_cast<std::uint32_t> (16), static_cast<std::uint32_t> (cores)
        );
...
void script_checker_queue::stop()
{
    ...
    {
        log_info(
            "Script checker queue is stopping, workers = " << total_workers_ <<
            "."
        );
        ...
        log_info(
            "Script checker queue is stopped, workers = " <<
            total_workers_ << "."
        );
    }
}
...
bool script_checker_queue::loop(const bool & is_main_thread)
{
    ...
    /**
     * Signal all workers that we are done.
     */
