...
bool block_merkle::is_valid_spv()
{
     ...
     * Check the timestamp.
     */
    if (m_block_header.timestamp > std::time(0) + constants::max_clock_drift)
    {
        log_error(
            "Block merkle timestamp too far in the future (" <<
    ...
    if (block::check_proof_of_work(m_hash, m_block_header.bits) == false)
    {
